...
(function ($) {
    $.jBox = function (content, options) {
        ...
        $.each(content, function (stateName, stateOptions) {
            ...
            states.push('<div class="jbox-button-panel" style="height:28px;padding:5px 0 5px 0;text-align: right;' + (withButtons ? '' : 'display:none;') + '">');
            if (!options.isTip) {
    ...
    $.jBox.tip = function (content, icon, options) {
        ...
        html.push('html:');
        if (options.icon == 'none'){
        	html.push('<div style="min-height:18px;height:auto;margin:10px;padding-top:0px;text-align:left;">');
        }else{
